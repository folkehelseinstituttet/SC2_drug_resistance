...
stanf_3clpro <- read_csv("N:/Virologi/JonBrate/Drug resistance SARSCOV2/data/2025.03.06_3CLpro_inhibitors.csv") %>% 
...
res <- vector("list", length = length(nanopore_files))
pb <- txtProgressBar(min = 0, max = length(nanopore_files), initial = 0, style = 3) 
for (i in seq_along(nanopore_files)) {
  try({
    res[[i]] <- read_tsv(nanopore_files[i], show_col_types = F) %>% 
...
# Match on the Nanopore sequence id
NSP5_BN_import <- left_join(samples, to_BN_NSP5, by = c("SEQUENCEID_NANO29" = "name")) %>% 
  # Keep relevant columns for BN import
